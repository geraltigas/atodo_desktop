...
import { set_now_doing_task, set_now_is_work_time, set_work_time } from '../../api/app_state_api'
...
const ClearTimeButton = () => {
  ...
  const handleConfirm = () => {
    ...
    Promise.all([p_work_time, p_now_is_work_time, p_now_doing_task]).then(
      ([work_time, now_is_work_time, now_doing_task]) => {
        batch(() => {
...
export const WorkControl = () => {
  ...
  const handleButtonClick = useCallback(() => {
    ...
    Promise.all([p_now_doing_task, p_complete_task])
      .then(() => {
        return get_schedule()
      })
      .then((schedule) => {
  ...
  return (
    <div className={styled.workControl}>
      ...
      <Button
        ...
        disabled={isDisabled || now_doing_task_signal.value === -1}
      >
        Complete Task
      </Button>
    </div>
